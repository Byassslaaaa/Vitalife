        ...
        <section class="pt-32 pb-20" aria-labelledby="hero-title">
            <div class="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
                <div class="text-left mb-12">
                    <h1 id="hero-title" class="text-4xl sm:text-5xl lg:text-6xl font-black text-gray-900 mb-4">SPA</h1>
                    <p class="text-lg lg:text-xl text-gray-700">Finding SPA location nearby you</p>
                </div>
            </div>
        </section>
        ...
        <main class="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-12">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="spa-listings">
                @foreach ($spaTotal as $index => $spa)
                    <article
                        class="spa-item bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 {{ $index >= 6 ? 'hidden' : '' }}"
                        ...
                        <a href="{{ route('spa.detail', $spa->id_spa) }}" class="block">
                            ...
                            <header class="p-6 border-b border-gray-100">
                                <h2 id="spa-name-{{ $spa->id_spa }}" class="text-xl font-bold text-gray-900 mb-2">
                                    {{ $spa->nama }}</h2>
                            ...
                            <div class="aspect-w-16 aspect-h-9">
                                <img src="{{ $spa->image ? asset($spa->image) : asset('image/spa-default.jpg') }}"
                                    alt="{{ $spa->nama }} spa interior" class="w-full h-48 object-cover"
                                    ...
                                    <div class="flex items-center justify-center">
                                        @if ($spa->is_open)
                                            <div class="flex items-center text-green-600">
                                                <div class="w-3 h-3 bg-green-400 rounded-full mr-2 animate-pulse">
                                                </div>
                                                <span class="text-sm font-medium">
                                                    @if ($spa->waktuBuka && is_array($spa->waktuBuka))
                                                        @php
                                                            ...
                                                            $todayHours =
                                                                $spa->waktuBuka[$indonesianDay] ?? 'Contact for hours';
                                                        @endphp
