                        ...
                        @foreach ($payments as $index => $payment)
                            <tr class="border-b">
                                ...
                                <td class="p-2">
                                    ...
                                    @else
                                        <span
                                            class="px-2 py-1 bg-gray-500 text-white rounded text-xs">{{ $payment->status }}</span>
                                    @endif
                                </td>
                                <td class="p-2">
                                    <select id="status-{{ $payment->kode }}" class="p-1 border rounded mr-2 text-sm">
                                        <option value="Menunggu" {{ $payment->status == 'Menunggu' ? 'selected' : '' }}>
                                            Menunggu</option>
                                        <option value="Dikonfirmasi"
                                            {{ $payment->status == 'Dikonfirmasi' ? 'selected' : '' }}>Dikonfirmasi
                                        </option>
                                        <option value="Ditolak" {{ $payment->status == 'Ditolak' ? 'selected' : '' }}>
                                            Ditolak</option>
                                    </select>
                                    <button onclick="updateStatus('{{ $payment->kode }}')"
                ...
                .then(payments => {
                    ...
                    payments.forEach((payment, index) => {
                        ...
                        if (payment.status === 'Menunggu') {
                            ...
                            statusBadge =
                                '<span class="px-2 py-1 bg-gray-500 text-white rounded text-xs">Menunggu</span>';
                        }
