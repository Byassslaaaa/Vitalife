...
@section('judul-halaman', 'Edit Layanan Gym')

@section('content')
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-2xl font-bold">Edit Layanan Gym</h1>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="gym_id" id="gym_id" required
                                ...
                                @foreach($gyms as $gym)
                                    <option value="{{ $gym->id_gym }}" {{ (old('gym_id', $service->gym_id) == $gym->id_gym) ? 'selected' : '' }}>
                                        {{ $gym->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="4" required
                                      class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200"
                                      placeholder="Jelaskan detail layanan ini...">{{ old('description', $service->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @if($service->image)
                                <div class="mb-2">
                                    <img src="{{ asset($service->image) }}" alt="{{ $service->name }}" class="w-20 h-20 object-cover rounded">
