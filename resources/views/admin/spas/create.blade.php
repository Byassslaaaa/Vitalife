...
@section('content')
    <div class="max-w-5xl mx-auto p-4 bg-white shadow-md rounded-lg">
        ...
        <form id="spaForm" action="{{ route('admin.spas.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-6">
                <!-- Informasi Dasar -->
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    @foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $hari)
                        <div class="flex items-center space-x-2 mt-2">
                            <label for="waktuBuka_{{ strtolower($hari) }}" class="w-20 text-sm font-medium">{{ $hari }}</label>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Services Spa (3 Services Utama dengan Foto)</h3>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div id="serviceImagePreview{{ $i }}" class="mt-2">
                                        <img src="/placeholder.svg?height=64&width=64" alt="Preview" class="w-16 h-16 object-cover rounded-full bg-gray-200 hidden border-2 border-gray-300">
                                    </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div>
                        ...
                        <div id="imagePreview" class="mt-2">
                            <img src="/placeholder.svg?height=128&width=128" alt="Preview" class="w-32 h-32 object-cover rounded-lg bg-gray-200 hidden">
                        </div>
                        <p class="mt-1 text-sm text-gray-500">Upload foto interior atau eksterior spa (JPG, PNG, GIF, max 2MB)</p>
                        @error('image')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="border-t pt-6">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="submit" id="submitBtn"
                            ...
                            <span id="submitText">Simpan Data Spa</span>
                            <svg id="loadingIcon" class="animate-spin -ml-1 mr-3 h-4 w-4 text-white hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <script>
        ...
        document.getElementById('spaForm').addEventListener('submit', function(e) {
            ...
            // Basic validation
            const requiredFields = ['nama', 'alamat', 'noHP'];
            let isValid = true;
            ...
            if (!mainImage.files[0]) {
                isValid = false;
                mainImage.classList.add('border-red-500');
            } else {
                mainImage.classList.remove('border-red-500');
            }
            ...
            if (!isValid) {
                ...
                submitText.textContent = 'Simpan Data Spa';
                loadingIcon.classList.add('hidden');
                
                alert('Harap lengkapi semua field yang wajib diisi termasuk 3 services dengan foto dan foto utama spa');
                return false;
            }
