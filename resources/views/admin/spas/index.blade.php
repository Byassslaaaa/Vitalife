                ...
                <div class="bg-white rounded-lg shadow p-6 text-center">
                    <h3 class="text-2xl font-bold text-indigo-600">
                        @php
                            $serviceCount = 0;
                            foreach($spas as $spa) {
                                // Count from JSON services column
                                if($spa->services && is_array($spa->services)) {
                                    $serviceCount += count($spa->services);
                                }
                                // Count from spaServices relationship
                                if($spa->spaServices) {
                                    $serviceCount += $spa->spaServices->count();
                                }
                            }
                        @endphp
                        {{ $serviceCount }}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($spas as $spa)
                            <div class="bg-gray-50 rounded-lg p-6 hover:shadow-md transition-shadow duration-200">
                                    ...
                                    <div class="text-right">
                                        ...
                                        <p class="text-sm font-medium text-indigo-600">
                                            @php
                                                ...
                                                // Count relationship services
                                                if($spa->spaServices) {
                                                    $totalServices += $spa->spaServices->count();
                                                }
                                            @endphp
                                ...
                                <!-- Services and Delete Buttons -->
                                <div class="flex space-x-2">
                                    @if(Route::has('admin.spas.services.index'))
                                    <a href="{{ route('admin.spas.services.index', $spa->id_spa) }}" 
                                       class="flex-1 bg-purple-500 hover:bg-purple-600 text-white text-center py-2 px-3 rounded text-sm font-medium transition-colors">
                                        <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <form action="{{ route('admin.spas.destroy', $spa->id_spa) }}" method="POST" 
                                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus spa ini?')" class="flex-1">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
