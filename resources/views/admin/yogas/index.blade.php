        ...
        @if($yogas->count() > 0)
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-white rounded-lg shadow p-6 text-center">
                    <h3 class="text-2xl font-bold text-purple-600">{{ $yogas->count() }}</h3>
                    <p class="text-gray-600 text-sm">Total Yoga</p>
                </div>
                <div class="bg-white rounded-lg shadow p-6 text-center">
                    <h3 class="text-2xl font-bold text-green-600">{{ $yogas->where('is_active', true)->count() }}</h3>
                    <p class="text-gray-600 text-sm">Yoga Aktif</p>
                </div>
                <div class="bg-white rounded-lg shadow p-6 text-center">
                    <h3 class="text-2xl font-bold text-blue-600">{{ $yogas->avg('harga') ? 'Rp ' . number_format($yogas->avg('harga'), 0, ',', '.') : 'N/A' }}</h3>
                        ...
                        @foreach($yogas as $yoga)
                            <div class="bg-gray-50 rounded-lg p-6 hover:shadow-md transition-shadow duration-200">
                                ...
                                <div class="flex items-center mb-4">
                                    @if($yoga->image)
                                        <img src="{{ asset($yoga->image) }}" alt="{{ is_array($yoga->nama) ? implode(', ', $yoga->nama) : $yoga->nama }}" 
                                    ...
                                    @endif
                                    <div class="flex-1">
                                        <h3 class="font-semibold text-gray-900">{{ is_array($yoga->nama) ? implode(', ', $yoga->nama) : $yoga->nama }}</h3>
                                ...
                                <div class="mb-4">
                                    <div class="flex items-start">
                                        ...
                                        <p class="text-sm text-gray-600">{{ Str::limit(is_array($yoga->alamat) ? implode(', ', $yoga->alamat) : $yoga->alamat, 60) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="mb-4">
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-sm text-gray-600">{{ is_array($yoga->noHP) ? implode(', ', $yoga->noHP) : $yoga->noHP }}</span>
                                    </div>
                                </div>
