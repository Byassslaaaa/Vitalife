    ...
    <div class="container mx-auto px-4">
        ...
        @if($gyms->count() > 0)
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white rounded-lg shadow p-6 text-center">
                    <h3 class="text-2xl font-bold text-green-600">{{ $gyms->where('is_open', true)->count() }}</h3>
                    <p class="text-gray-600 text-sm">Gym Buka</p>
                </div>
                <div class="bg-white rounded-lg shadow p-6 text-center">
                    <h3 class="text-2xl font-bold text-red-600">{{ $gyms->where('is_open', false)->count() }}</h3>
                        ...
                        @foreach($gyms as $gym)
                            <div class="bg-gray-50 rounded-lg p-6 hover:shadow-md transition-shadow duration-200">
                                ...
                                <div class="mt-2 flex space-x-2">
                                    <form action="{{ route('admin.gyms.toggle-status', $gym->id_gym) }}" method="POST" class="flex-1">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" 
                                    ...
                                    <form action="{{ route('admin.gyms.destroy', $gym->id_gym) }}" method="POST" 
                                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus gym {{ $gym->nama }}?');" class="flex-1">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
            ...
            <div class="mt-6 bg-white rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-4">
                        <a href="{{ route('admin.gym-details.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                            Kelola Detail Halaman →
                        </a>
                        <a href="{{ route('admin.services.index') }}" class="text-purple-600 hover:text-purple-800 text-sm font-medium">
                            Kelola Layanan →
                        </a>
                    </div>
                </div>
        ...
        @else
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="p-12 text-center">
                    ...
                    <a href="{{ route('admin.gyms.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg font-medium inline-flex items-center">
    ...
    <script>
        ...
        document.querySelectorAll('form[onsubmit*="confirm"]').forEach(function(form) {
            form.addEventListener('submit', function(e) {
                ...
                if (confirm(`Apakah Anda yakin ingin menghapus gym "${gymName}"?\n\nTindakan ini tidak dapat dibatalkan dan akan menghapus semua data terkait.`)) {
                    this.submit();
                }
            });
        });
